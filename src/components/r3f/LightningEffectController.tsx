...
export const LightningEffectController: React.FC<LightningEffectControllerProps> = ({ targetPosition, onComplete }) => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < strikeCount; i++) {
      ...
      const individualRayOrigin = proposedTargetPos.clone().setY(proposedTargetPos.y + rayOriginYOffset);
      ...
      if (hit) {
        ...
        const threeHitPoint = new THREE.Vector3(rapierHitPoint.x, rapierHitPoint.y, rapierHitPoint.z);
    ...
    if (targets.length > 0) {
      console.log(`Controller found ${targets.length} actual targets near`, targetPosition);
      setActualTargets(targets);

      // Calculate start position based on the average of *actual* hit targets
      const averageTarget = targets.reduce((acc, pos) => acc.clone().add(pos), new THREE.Vector3(0, 0, 0)).divideScalar(targets.length);
      const commonStart = averageTarget.clone().add(new THREE.Vector3(0, rayOriginYOffset, 0)); // Start above average hit point
  ...
  return <LightningStrike commonStartPosition={startPosition} targetPositions={actualTargets} />;
};
